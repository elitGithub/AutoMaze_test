...
use Libraries\Monolog\Handler\FormattableHandlerInterface;
use Libraries\Monolog\Handler\Handler;
use Libraries\Monolog\Handler\HandlerInterface;
use Libraries\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy;
use Libraries\Monolog\Handler\FingersCrossed\ActivationStrategyInterface;
use Libraries\Monolog\Handler\ProcessableHandlerInterface;
use Libraries\Monolog\Handler\ProcessableHandlerTrait;
use Libraries\Monolog\Logger;
use Libraries\Monolog\ResettableInterface;
 ...
 *
 * @phpstan-import-type Record from \Libraries\Monolog\Logger
 * @phpstan-import-type Level from \Libraries\Monolog\Logger
 * @phpstan-import-type LevelName from \Libraries\Monolog\Logger
 */
class FingersCrossedHandler extends Handler implements ProcessableHandlerInterface, ResettableInterface, FormattableHandlerInterface
{
     ...
     */
    protected $handler;
    /** @var \Libraries\Monolog\Handler\FingersCrossed\ActivationStrategyInterface */
     ...
     * @param int|string|\Libraries\Monolog\Handler\FingersCrossed\ActivationStrategyInterface  $activationStrategy Strategy which determines when this handler takes action, or a level name/value at which the handler is activated
    ...
    public function __construct($handler, $activationStrategy = null, int $bufferSize = 0, bool $bubble = true, bool $stopBuffering = true, $passthruLevel = null)
    {
        ...
        if (!$this->handler instanceof HandlerInterface && !is_callable($this->handler)) {
            throw new \RuntimeException("The given handler (".json_encode($this->handler).") is not a callable nor a Libraries\Monolog\Handler\HandlerInterface object");
        }
    }
